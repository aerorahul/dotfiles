...
class HPSS(object):
    ...
    def check(self, exit_codes):
        ...
        for exit_code in exit_codes:
            if (exit_code):
                print(f'unexpected error, exit code = {exit_code}')
                sys.exit(exit_code)
    ...
    def command(self, input_files, copy=False, get=False, move=False):
        ...
        if (get):
            self.get(input_files)
        elif (copy):
            self.copy(input_files)
        elif (move):
            self.move(input_files)
    ...
    def stage(self, input_files):
        ...
        for files in input_files:
            ...
            if (str_len > 3072):
                ...
                subset_tmp_files = [tmp_files[x:x+nfiles]
                                    for x in range(0, len(tmp_files), nfiles)]
                for subset in subset_tmp_files:
                    cmd = '%s stage -w %s' % (self.hsi, ' '.join(subset))
                    process.append(sp.Popen(shlex.split(
                        cmd), stderr=sp.PIPE, stdout=sp.PIPE))
            else:
    ...
    def get(self, input_files):
        ...
        for files in input_files:
            for filename in files:
                ...
                cmd = '%s -xf %s %s' % (self.htar,
                                        filename[0], ' '.join(filename[1:]))
                process.append(sp.Popen(shlex.split(
                    cmd), stderr=sp.PIPE, stdout=sp.PIPE))
    ...
    def copy(self, input_files):
        ...
        for files in input_files:
            for filename in files:
                ...
                cmd = '%s mkdir -p %s' % (self.hsi,
                                          os.path.dirname(filename[1]))
                hsi = sp.Popen(shlex.split(
                    cmd), stderr=sp.PIPE, stdout=sp.PIPE)
                exit_code = hsi.wait()
                self.check([exit_code])

                cmd = '%s copy -pf %s %s' % (self.hsi,
...
def make_dict(input_args):
    ...
    if (input_args.read_dict):
        ...
        for volume in dict_vol.keys():
            if (input_args.write_dict):
                write_dict('%s.%s.dict' %
                           (input_args.filename, volume), dict_vol[volume])

    print()
    print('Your files are spread across the following volumes:')
    print('Volume     # of files')
    for volume in dict_vol.keys():
        print(f'{volume:>10} {len(dict_vol[volume])}')
    print()
    ...
    # skip volumes
    for volume in dict_vol.keys():
        if volume in input_args.skip_volumes:
            dict_vol.pop(volume, None)
...
def sort_dict(input_files):
    ...
    for filename in input_files:
        ...
        o, e = grep.communicate()
        volume = re.findall(r'PV List: (\S+)', o)[0]
        if (volume not in dict_vol):
            dict_vol[volume] = []
        dict_vol[volume].append([filename[0], ' '.join(filename[1:])])

    # Order the dictionary of volumes by number of files in each volume
    o_dict_vol = OrderedDict(sorted(list(dict_vol.items()), key=lambda k_v: len(k_v[1]), reverse=True))
...
def main():
    ...
    parser = ArgumentParser(description='smartly get/copy/move files from or within HPSS',
                            formatter_class=ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        '-f', '--filename', help='filename containing list of files to get/copy/move on HPSS', required=True)
    parser.add_argument('-v', '--verbose', help='verbose',
                        action='store_true', required=False)
    parser.add_argument('-s', '--skip_volumes', type=str,
                        nargs='+', default=[], required=False)
    parser.add_argument('-n', '--nsimul', help='number of simultaneous HPSS operations',
                        type=int, default=1, required=False)
    parser.add_argument('-w', '--write_dict', help='write sorted dictionary',
                        action='store_true', required=False)
    parser.add_argument('-r', '--read_dict', help='read sorted dictionary, if so, skip sorting',
                        action='store_true', required=False)
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('-g', '--get', help='get',
                       action='store_true', required=False)
    group.add_argument('-c', '--copy', help='copy',
                       action='store_true', required=False)
    group.add_argument('-m', '--move', help='move',
                       action='store_true', required=False)
    ...
    for volume in dict_vol.keys():
        ...
        print(f'Volume: {volume} ( {len(dict_vol[volume])} )')
